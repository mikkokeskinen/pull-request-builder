...
var before = exports.before = async function before(json) {
  return json.ref && json.ref.startsWith('refs/tags') ? initiate(json) : undefined;
};

//
//
var after = exports.after = async function after(json) {
  return json.source.buildspec === "carryspec.yml" ? complete(json) : undefined;
};
...
var initiate = async function initiate(json) {
  ...
  var rel = json.ref.replace('refs/tags/', '');
  var issue = await (0, _github2.raiseIssue)(json.repository.full_name, 'release ' + rel + ' to live');
  console.log("==> issue", JSON.stringify(issue));

  var pr = issue.response.body.number;
  var codebuild = await (0, _codebuild.carryspec)(json.repository.full_name, pr, json.after);
...
var complete = async function complete(json) {
  ...
  if (status.success) {
    ...
    console.log("==> github", JSON.stringify(github));
    var issue = await (0, _github2.closeIssue)((0, _codebuild.buildRepo)(json), (0, _codebuild.buildPR)(json));
    console.log("==> issue", JSON.stringify(issue));
  } else {
